...
app.get("*", function (request, response) {
    ...
    const options = { host: settings.RPC.host, port: settings.RPC.port, method: request.method, path: request.path, headers: request.headers };
    var webProxy = http.request(options, function (proxy_res) {
        // Copy specific headers from the proxy response to the actual response
        if (proxy_res.headers['content-length']) {
            response.setHeader('content-length', proxy_res.headers['content-length']);
        }

        if (proxy_res.headers['content-security-policy']) {
            response.setHeader('content-security-policy', proxy_res.headers['content-security-policy']);
        }
        ...
        // Set the status code of the actual response to match the proxy response
        response.status(proxy_res.statusCode);
